...
game_rewards_table = {
        events.COIN_COLLECTED: 30,
        events.BOMB_DROPPED: -10,
        #events.KILLED_OPPONENT: 100,
        #events.GOT_KILLED: -300,
        events.KILLED_SELF: -300,
        #events.CRATE_DESTROYED: 5,
        #events.SURVIVED_ROUND: 5,
        #events.OPPONENT_ELIMINATED: 200,
        events.MOVED_UP: 1,
        events.MOVED_DOWN: 1,
        events.MOVED_LEFT: 1,
...
def get_state_matrix(state):
    ...
    return np.stack((field, player_map, enemy_map, coin_map, danger_map), axis=2)
...
def create_model():
    model = tf.keras.models.Sequential([
        tf.keras.layers.Input(shape=(17, 17, 5)),
    ...
    model.compile(loss='mse', optimizer=tf.keras.optimizers.Adam(lr=0.003), metrics=['accuracy'])
    return model